...
from typing import TYPE_CHECKING, Generic, Tuple, TypeVar, overload, cast
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    class _CallableWithDefault(Protocol[_Self_co, _K_contra, _V_co, _D_contra]):
        ...
        @overload
        def __call__(self: _Self_co, key: _K_contra, default: _D_contra) -> _V_co | _D_contra: ...

else:
    _CallableWithDefault = object
...
def with_default(
    ...
    class _WithDefaultDescriptor:
        def __init__(self, func: Callable[[_Self, _K], _V]) -> None:
            self.func = func
        ...
        def __get__(self, instance: _Self | None, owner: type[_Self] | None = None) -> _CallableWithDefault[_Self, _K, _V, _D]:
            if instance is None:
                return self  # type: ignore[return-value]

            @overload
            def bound_method(key: _K) -> _V: ...

            @overload
            def bound_method(key: _K, default: _D) -> _V | _D: ...

            def bound_method(key: _K, default: _D | _NoDefault = _NO_DEFAULT) -> _V | _D:
                try:
                    return self.func(instance, key)
                except KeyError:
    ...
    return _WithDefaultDescriptor(meth)  # type: ignore[return-value]
