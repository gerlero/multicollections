...
def test_popitem_method(cls: type[MutableMultiMapping]) -> None:
    ...
    # Test empty MultiDict
    empty_md = cls()
    with pytest.raises((StopIteration, KeyError)):  # Different implementations may raise different errors
        empty_md.popitem()
...
@pytest.mark.parametrize("cls", [MultiDict, ListMultiDict])  # multidict has type restrictions
